...
func TestSecureHeaders(t *testing.T) {
	...
	// Create a mock HTTP handler that we can pass our secureHeaders() middleware, which writes a 200 status code and an "OK" response body
	next := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte("OK"))
	})

	// Pass the mock HTTP handler to our secureHeaders middleware. Because secureHeaders returns a http.Handler, we can call its ServeHTTP() method,
