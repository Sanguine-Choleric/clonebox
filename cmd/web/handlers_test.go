...
func TestMatchesMimeType(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Mimetype with Parameters Miss",
			args: args{
				pattern:  "text/plain",
		...
		{
			name: "Wildcard Mimetype with Parameters",
			args: args{
				pattern:  "text/*",
				mimeType: "text/plain; charset=utf-8",
			},
		...
		{
			name: "Wildcard Whitespace Trim Test",
			args: args{
				pattern:  "  image/*  ",
				mimeType: "image/jpeg",
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := app.matchesMimeType(tt.args.pattern, tt.args.mimeType); got != tt.want {
